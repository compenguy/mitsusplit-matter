...
-include local.mak
ESP_TARGET ?= esp32c3
ESP_MATTER_DEVICE ?= esp32c3_t_rsc3
ESP_TTY ?=
WIFI_SSID ?=
WIFI_PSK ?=
...
USER_ID = $(shell id -u)
GROUP_ID = $(shell id -g)
TTY_GROUP_NAME = $(if $(ESP_TTY),$(shell stat --format=%G $(ESP_TTY)),dialout)
TTY_GROUP_ID = $(if $(ESP_TTY),$(shell stat --format=%g $(ESP_TTY)),dialout)
$(info ESP_TTY=$(ESP_TTY))
$(info TTY_GROUP_NAME=$(TTY_GROUP_NAME))
...
monitor: build/.firmware-flash.stamp | bootstrap docker-build
	$(V)$(call docker_run,source $(ESP_ENV_SCRIPT) && idf.py --build-dir \$${ESP_MATTER_PATH}/build --project-dir \$${ESP_MATTER_PATH}/src monitor -p $(ESP_TTY))
